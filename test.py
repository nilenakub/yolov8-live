...
# Take Image from Webcam
    ...
    results = model.predict(frame, show=True, classes=[5, 7], conf=0.5, iou=0.2)
...
# Calculate Width & Height of Bounding Boxes 
    for result in results:
        ...
        # Check for "Clear plastic bottle" before processing "Plastic bottle cap"
        clear_bottle_detected = any(class_names[int(box.cls[0])] == "Clear plastic bottle" for box in boxes)

        if clear_bottle_detected:
            for box in boxes:
                x, y, w, h = box.xywh[0]
